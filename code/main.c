...
KestrelPrintHelp(VOID)
{
    ...
    wprintf(
        ...
        L"  --archaeology  Orphaned dangerous ACEs (dead owners, live rights) + stale privileged accounts\n"
    ...
    _Out_ KESTREL_CONFIG *pCfg)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (_wcsicmp(arg, L"--archaeology") == 0) {
            pCfg->bRunArchaeology = TRUE;
            pCfg->bExplicitModules = TRUE;
            continue;
        }
...
int wmain(int argc, wchar_t *argv[])
{
    ...
    if (cfg.bRunArchaeology) {
        wprintf(L"\n═══ Kestrel — AD Archaeology ═══\n");
        hr = KestrelRunArchaeologyScan(wszDomainNC);
        if (FAILED(hr))
            wprintf(L"[!] KestrelRunArchaeologyScan failed: 0x%08X\n", hr);
    }
